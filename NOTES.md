...
`loopCopyIn`'s progress display (`dirStats`/`cpRecursiveWithProgress`, `loopfs.go`) tracks bytes actually written, not file count. A directory holding one large file among many small ones would otherwise hit "N/N files" almost as soon as `cp` opens that last file for writing, long before its content is done copying, and then sit at a misleading 100% for however long that file takes. The displayed percentage only reaches 100% once the underlying `cp` process has genuinely finished.
